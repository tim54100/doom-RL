...
def make_env():
    env_spec = gym.spec('ppaquette/DoomDeathmatch-v0')
    env_spec.id = 'Deathmatch_v0'
    env = env_spec.make()
...
def deep_q_learning(sess,
    ...
    for i_episode in range(num_episodes):
	    ...
	    if done and total_reward<20:
		reward=-10
	    elif done:
		pass
	    elif info['HEALTH'] != health:
		reward+=(info['HEALTH']-health)/100
	        health=info['HEALTH']
		...
		health=100
                ...
                print('noop %d, shoot %d, right %d, left %d\n forward %d turn_right %d turn_left %d' % \
                    (counts_action[0], counts_action[1], counts_action[2], counts_action[3], counts_action[4],
		     counts_action[5], counts_action[6]))
                break
